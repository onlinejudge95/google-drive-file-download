...
try:
    ...
    flags.add_argument(
        "--log-level", type=str, help="Choose a log level", default="INFO"
    )
...
def get_credentials():
    ...
    credential_path = os.path.join(
        credential_dir, "download-from-google-drive-to-s3.json"
    )
...
def we_should_process_this_file(filename, match_files):
    if (
        not match_files
    ):  # We have not supplied any file names to match against, so process everything.
        return True
...
def main():
    ...
    log_handler = FileHandler(
        log_filename, mode="w", level=args.log_level, bubble=True
    )
    stdout_handler = StreamHandler(
        sys.stdout, level=args.log_level, bubble=True
    )
        ...
        with log_handler.applicationbound():
            ...
            while request is not None:
                ...
                if page_counter >= args.start_page:
                    log.info("######## Page {} ########".format(page_counter))

                    for this_file in file_page["files"]:
                        file_counter += 1
                        page_file_counter += 1
                        if we_should_process_this_file(
                            this_file["name"], match_filenames
                        ):
                            log.info(
                                "#== Processing {} file number {} on page {}. {} files processed.".format(
                                    this_file["name"],
                                    page_file_counter,
                                    page_counter,
                                    file_counter,
                            ...
                            # download the file
                            download_request = drive_service.files().get_media(
                                fileId=this_file["id"]
                            )
                            fh = (
                                io.BytesIO()
                            )  # Using an in memory stream location
                ...
                if args.end_page and page_counter == args.end_page:
                    log.info("Finished paging at page {}".format(page_counter))
                    break
